 ...
 *
 * - TS2322: TYPE_NOT_ASSIGNABLE for Snippet type compatibility
 *   - tsgo loses `unique symbol` when accessed through a property
 *   - svelte2tsx generates Snippet types with `unique symbol` that tsgo can't match
 *   - tsc handles these correctly
...
const DiagnosticCode = {
  ...
  // [tsgo-specific] Snippet type compatibility
  // tsgo loses `unique symbol` when accessed through property, causing Snippet type mismatch
  // Issue: https://github.com/microsoft/typescript-go/issues/1682
...
/**
 * Check if error is a Snippet type mismatch from tsgo's unique symbol issue
 *
 * tsgo loses `unique symbol` when accessed through a property, causing errors like:
 * "Type '(props: {...}) => {...} & unique symbol' is not assignable to type 'Snippet<[...]>'"
 *
 * Issue: https://github.com/microsoft/typescript-go/issues/1682
 */
function isSnippetTypeMismatch(d: Diagnostic): boolean {
  // Only filter in .svelte.tsx files (generated by svelte2tsx)
  if (!d.file.endsWith(".svelte.tsx")) {
    return false;
  }

  // Check for Snippet type and unique symbol pattern in error message
  return d.message.includes("Snippet<") && d.message.includes("unique symbol");
}
